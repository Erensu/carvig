/*------------------------------------------------------------------------------
* postpos.cc : post-processing positioning
*
...
static int execses(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt,
                   const solopt_t *sopt, filopt_t *fopt, int flag,
                   ...
                   const char *rov, const char *base)
{
    ...
    if (ts.time!=0&&te.time!=0&&tu>=0.0) {
        ...
        for (i=0;;i++) { /* for each periods */
            ...
            /* execute processing session */
            stat=execses_b(tts,tte,ti,popt,sopt,&fopt_,flag,ifile,index,nf,ofile,
                           rov,base);
        ...
        /* execute processing session */
        stat=execses_b(ts,te,ti,popt,sopt,&fopt_,1,ifile,index,n,ofile,rov,
                       base);
        ...
        /* execute processing session */
        stat=execses_b(ts,te,ti,popt,sopt,&fopt_,1,infile,index,n,outfile,rov,
                       base);
    }
