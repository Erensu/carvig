                       ...
                       sta_t *sta,sigind_t *sigind)
{
    ...
    /* save signal index */
    if (sigind) {
        for (j=0;j<7;j++) sigind[j]=index[j];
    }
    /* read record */
    while (fgets(buff,MAXRNXLEN,fp)) {
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
    ...
    /* read rinex obs data body */
    while ((n=readrnxobsb(fp,opt,ver,tsys,tobs,&flag,data,sta,
                          obs->sind[rcv<=2?rcv-1:0]))>=0&&stat>=0) {
...
extern int input_rnxctr(rnxctr_t *rnx, FILE *fp)
{
    ...
    if (rnx->type=='O') {
        if ((n=readrnxobsb(fp,rnx->opt,rnx->ver,&rnx->tsys,rnx->tobs,&flag,
                           rnx->obs.data,&rnx->sta,rnx->obs.sind[0]))<=0) {
            rnx->obs.n=0;
            return n<0?-2:0;
        }
...
extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    if (opt->rnxver<=2.99) { /* ver.2 */
        ...
        if      (opt->navsys==SYS_GPS) strcpy(sys,"G: GPS");
        else if (opt->navsys==SYS_GLO) strcpy(sys,"R: GLONASS");
        else if (opt->navsys==SYS_GAL) strcpy(sys,"E: Galielo");
        else if (opt->navsys==SYS_QZS) strcpy(sys,"J: QZSS");   /* ver.3.02 */
        else if (opt->navsys==SYS_CMP) strcpy(sys,"C: BeiDou"); /* ver.3.02 */
        else if (opt->navsys==SYS_IRN) strcpy(sys,"I: IRNSS");  /* ver.3.03 */
        else if (opt->navsys==SYS_SBS) strcpy(sys,"S: SBAS Payload");
        else strcpy(sys,"M: Mixed");
    }
