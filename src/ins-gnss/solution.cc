...
extern void insstatocov(const insopt_t *opt,const insstate_t *ins,double *Pa,
                        double *Pv,double *Pp)
{
    int i,j;
...
static int decode_solposmode(char *buff)
{
    ...
    if (strstr(buff,GNSPOSSTR)) return 1;
    if (strstr(buff,INSPOSSTR)) return 2;
    if (strstr(buff,VOSPOSSTR)) return 3;
    if (strstr(buff,GTHPOSSTR)) return 4;
    return 0;
}
/* decode x/y/z-ecef ---------------------------------------------------------*/
static int decode_solxyz(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    if (i<n) sol->ista=(unsigned char)val[i++];
    ...
    if (i<n&&opt->wlratio) sol->wlratio=(float)val[i++];
...
static int decode_solvo(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    if ((n=tonum(buff,sep,val))<3) return 0;
    if (i+3<=n) {
        for (j=0;j<3;j++) sol->rr[j]=val[i++]; /* xyz */
    }
    if (i+3<=n) {
        sol->rr[3]=val[i]; i++; /* vel-x */
        sol->rr[4]=val[i]; i++; /* vel-y */
        sol->rr[5]=val[i]; i++; /* vel-z */
    }
    if (i+3<=n) {
        sol->att[0]=val[i]; i++; /* roll */
        sol->att[1]=val[i]; i++; /* pitch */
        sol->att[2]=val[i]; i++; /* yaw */
    }
    sol->stat=SOLF_VO;
    return 1;
}
...
static int decode_solins(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    if (opt->ins_posf==SOLF_XYZ) {
        if (i+3<=n) {
            for (j=0;j<3;j++) sol->rr[j+3]=val[i++]; /* xyz */
        }
        if (i+3<=n) {
            for (j=0;j<9;j++) P[j]=0.0;
            P[0]=val[i]*val[i]; i++; /* sdx */
    ...
    if (i+3<=n&&opt->outatt) for (j=0;j<3;j++) sol->att[j]=val[i++]; /* attitude */
    if (i+3<=n&&opt->outatt) for (j=0;j<3;j++) sol->qa[j] =(float)val[i++]; /* attitude variance */
    if (i+3<=n&&opt->outvb ) for (j=0;j<3;j++) sol->vb[j]=val[i++];  /* vehicle velocity */
    if (i+3<=n&&opt->outacc) for (j=0;j<3;j++) sol->ab[j]=val[i++];  /* vehicle acceleration */
    if (i+3<=n&&opt->outba ) for (j=0;j<3;j++) sol->ba[j]=val[i++];  /* acceleration bias */
    if (i+3<=n&&opt->outbg ) for (j=0;j<3;j++) sol->bg[j]=val[i++];  /* gyro bias */
    if (i+3<=n&&opt->odo   ) for (j=0;j<3;j++) sol->vr[j]=val[i++];  /* odometry velocity in rear frame */
    if (i+1<=n&&opt->odo   ) for (j=0;j<1;j++) sol->os   =val[i++];  /* odometry scale factor */
    if (i+4<=n&&opt->outclk) for (j=0;j<4;j++) sol->dtr[j]=val[i++]; /* receiver clock bias to time systems (s) */
    if (i+3<=n&&opt->dopp  ) for (j=0;j<3;j++) sol->dv[j]=val[i++];  /* doppler velocity expressed in ecef */
    if (i+1<=n&&opt->dopp  ) for (j=0;j<1;j++) sol->dtrr =val[i++];  /* receiver clock drift (m/s) about GPST */
    if (i+6<=n&&opt->outimuraw) {
        for (j=0;j<3;j++) sol->imu.gyro[j]=val[i++]; /* imu gyro measurement data */
        for (j=0;j<3;j++) sol->imu.accl[j]=val[i++]; /* imu accl measurement data */
    }
    for (i=0;i<3;i++) sol->qa[i]=SQR(sol->qa[i]);
    return 1;
}
...
static int decode_groundtruth(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    *sol=sol0;
    ...
    if ((n=tonum(p,sep,val))<3) return 0;
    if (i+3<=n) {
        for (j=0;j<3;j++) sol->rr[j]=val[i++];
    }
    sol->stat=SOLF_GRTH;
    return 1;
}
...
static int decode_solpos(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    *sol=sol0;
    ...
    switch (opt->posf) {
        ...
        case SOLF_INS : return decode_solins(p,opt,sol);
        case SOLF_VO  : return decode_solvo (p,opt,sol);
    }
    return 0;
}
...
*-----------------------------------------------------------------------------*/
extern int inputsolx(unsigned char data, gtime_t ts, gtime_t te, double tint,
                     int qflag, solbuf_t *solbuf)
{
    ...
    switch (stat) {
        case 0: solopt=&solopt_default;     break;
        case 1: solopt=&solopt_default;     break;
        case 2: solopt=&solopt_ins_default; break;
        case 3: solopt=&solopt_vo_default;  break;
        case 4: solopt=&solopt_default;     goto ground_truth;
    }
    /* decode solution */
    sol.time=solbuf->time;
...
extern int readsolx(char *files[], int nfile, gtime_t ts, gtime_t te,
...
extern void freeposebuf(posebuf_t *posebuf)
{
    ...
    if (posebuf->data) free(posebuf->data);
...
extern int addpose(posebuf_t *posebuf, const pose_meas_t *pose)
{
    ...
    if (posebuf->cyclic) { /* ring buffer */
        if (posebuf->nmax<=1) return 0;
        posebuf->data[posebuf->end]=*pose;
        if (++posebuf->end>=posebuf->nmax) posebuf->end=0;
        if (posebuf->start==posebuf->end) {
            if (++posebuf->start>=posebuf->nmax) posebuf->start=0;
        }
        else posebuf->n++;
    ...
    if (posebuf->n>=posebuf->nmax) {
        ...
        if (!(solbuf_data=(pose_meas_t *)realloc(posebuf->data,sizeof(pose_meas_t)*posebuf->nmax))) {
            ...
            free(posebuf->data); posebuf->data=NULL; posebuf->n=posebuf->nmax=0;
            return 0;
        }
...
/* output vo solution---------------------------------------------------------*/
static int outvo(unsigned char *buff,const sol_t *sol,const char *s,
                 const solopt_t *opt)
{
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;
    ...
    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f",s,sep,sol->rr[0],sep,sol->rr[1],sep,sol->rr[2]); /* output position */
    p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f",sep,sol->rr[3],sep,sol->rr[4],sep,sol->rr[5]); /* output velocity */
    p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f",sep,sol->att[0]*R2D,sep,sol->att[1]*R2D,sep,sol->att[2]*R2D); /* output attitude */

    if (outmoni) p+=sprintf(p," %s",VOSPOSSTR);
    p+=sprintf(p,"\n");
    return p-(char *)buff;
}
/* output ins solution -------------------------------------------------------*/
static int outins(unsigned char *buff,const sol_t *sol,const char *s,
                  const solopt_t *opt)
{
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;
    ...
    if (opt->ins_posf==SOLF_LLH) {
        ...
        p+=sprintf(p,"%s%10.4f%s%3d%s%3d%s%4d%s%10.4f%s%10.4f%s%10.4f%s%10.4f%s%10.4f%s%10.4f%s%6.2f%s%6.1f",
                   sep,pos[2],sep,sol->stat,sep,sol->ista,sep,sol->ns,sep,SQRT(Qp[4]),sep,
                   SQRT(Qp[0]),sep,SQRT(Qp[8]),sep,sqvar(Qp[1]),sep,sqvar(Qp[2]),
                   sep,sqvar(Qp[5]),sep,sol->age,sep,0.0);

        if (opt->outvel) { /* output velocity */
            ecef2enu(pos,sol->rr+3,venu);
            p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f",
                       sep,venu[1],sep,venu[0],sep,venu[2],sep,SQRT(Qv[4]),sep,
        ...
        p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%4d%s%3d%s%10.4f%s%10.4f%s%10.4f%s%10.4f%s%10.4f%s%10.4f%s%6.2f%s%6.1f",
                   ...
                   sol->ns,sep,SQRT(Pp[0]),sep,SQRT(Pp[4]),sep,SQRT(Pp[8]),
                   sep,sqvar(Pp[1]),sep,sqvar(Pp[2]),sep,sqvar(Pp[5]),
                   sep,sol->age,sep,sol->ratio);

        if (opt->outvel) { /* output velocity */
            p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f",
                       sep,sol->rr[3],sep,sol->rr[4],sep,sol->rr[5],sep,
                       SQRT(Pv[0]),sep,SQRT(Pv[4]),sep,SQRT(Pv[8]),
                       sep,sqvar(Pv[1]),sep,sqvar(Pv[2]),sep,sqvar(Pv[5]));
        }
    }
    ...
    if (opt->odo) {
        p+=sprintf(p,"%s%10.6lf%s%10.6lf%s%10.6lf%s%10.6lf",sep,sol->vr[0],sep,sol->vr[1],sep,sol->vr[2],sep,sol->os);
    }
    if (opt->outclk) {
        p+=sprintf(p,"%s%15.6lf%s%15.6lf%s%15.6lf%s%15.6lf",
                   sep,sol->dtr[0]*1E3,sep,sol->dtr[1]*1E3,sep,sol->dtr[2]*1E3,sep,sol->dtr[3]*1E3);
    }
    if (opt->dopp) {
        p+=sprintf(p,"%s%10.6lf%s%10.6lf%s%10.6lf%s%10.6lf",
                   sep,sol->dv[0],sep,sol->dv[1],sep,sol->dv[2],sep,sol->dtrr);
    }
    if (opt->outimuraw) {
...
extern int outsolheads(unsigned char *buff, const solopt_t *opt)
{
    ...
    if (opt->posf==SOLF_VO) goto vohead;
    ...
    if (opt->outhead) {
        ...
        if      (opt->posf==SOLF_XYZ) p+=sprintf(p,"x/y/z-ecef=WGS84");
            ...
            if      (opt->ins_posf==SOLF_XYZ) p+=sprintf(p,"x/y/z-ecef=WGS84");
            else if (opt->ins_posf==SOLF_LLH) p+=sprintf(p,"lat/lon/height=%s/%s",s1[opt->datum],s2[opt->height]);
        }
    ...
    if (opt->posf==SOLF_LLH||(opt->posf==SOLF_INS&&opt->ins_posf==SOLF_LLH)) { /* lat/lon/hgt */
        if (opt->degf) {
        ...
        if (opt->outclk&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%15s%s%15s%s%15s%s%15s",
                       sep,"clk-gps(ms)",sep,"clk-glo-gps(ms)",sep,"clk-gal-gps(ms)",sep,"clk-cmp-gps(ms)");
        }
        if (opt->dopp&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%10s%s%10s%s%10s%s%10s",
                       sep,"dopp-vx(m/s)",sep,"dopp-vy(m/s)",sep,"dopp-vz(m/s)",sep,"clk-dri(m/s)");
        }
        if (opt->outimuraw&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%12s%s%12s%s%12s%s%12s%s%12s%s%12s",
                       sep,"gyrox(deg)",sep,"gyroy(deg)",sep,"gyroz(deg)",
                       sep,"acclx(m/s^2)",sep,"accly(m/s^2)",sep,"acclz(m/s^2)");
        }
    }
    else if (opt->posf==SOLF_XYZ||(opt->posf==SOLF_INS&&opt->ins_posf==SOLF_XYZ)) { /* x/y/z-ecef */
        p+=sprintf(p,"%14s%s%14s%s%14s%s%3s%s%4s%s%3s%s%10s%s%10s%s%10s%s%10s%s%10s%s%10s%s%6s%s%6s",
        ...
        if (opt->outba&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%10s%s%10s%s%10s",
                       sep,"bax(m/s^2)",sep,"bay(m/s^2)",sep,"baz(m/s^2)");
        }
        if (opt->outbg&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%10s%s%10s%s%10s",
                       sep,"bgx(rad/s)",sep,"bgy(rad/s)",sep,"bgz(rad/s)");
        }
        if (opt->odo&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%10s%s%10s%s%10s%s%10s",
                       sep,"vox(m/s)",sep,"voy(m/s)",sep,"voz(m/s)",sep,"odo-s");
        }
        if (opt->outclk&&opt->posf==SOLF_INS) {
            p+=sprintf(p,"%s%15s%s%15s%s%15s%s%15s",
                       sep,"clk-gps(ms)",sep,"clk-glo-gps(ms)",sep,"clk-gal-gps(ms)",sep,"clk-cmp-gps(ms)");
        ...
        p+=sprintf(p,"%14s%s%14s%s%14s%s%3s%s%4s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s",
                   ...
                   "Q",sep,"Qins",sep,"ns",sep,"sde(m)",sep,"sdn(m)",sep,"sdu(m)",sep,
...
extern int outsols(unsigned char *buff, const sol_t *sol, const double *rb,
                   const solopt_t *opt,const insstate_t *ins,
                   const insopt_t *insopt,int type)
{
    ...
    if (opt->posf==SOLF_VO) goto vosols;
...
vosols:
    ...
    switch (opt->posf) {
        ...
        case SOLF_INS:  p+=outins(p,sol,s,opt); break;
        case SOLF_VO :  p+=outvo (p,sol,s,opt); break;
    }
    return p-buff;
}
