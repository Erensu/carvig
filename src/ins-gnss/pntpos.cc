                   ...
                   double *resp, int *ns)
{
    ...
    if (ins) {
        insp2antp(ins,rr);
        for (i=0;i<4;i++) x[irc+i]=ins->dtr[i]*CLIGHT;
    }
    else {
...
#endif
            if (nrc) H[irc+nv*nx]=1.0;
            if (nla) {
                for (j=ila;j<ila+nla;j++) H[j+nv*nx]=dpdl[j-ila];
            }
            for (j=IP;j<IP+NP;j++) H[j+nv*nx]=e[j-IP];
            for (j=IA;j<IA+NA;j++) H[j+nv*nx]=dpda[j-IA];
        }
        else {
            for (j=0;j<nx;j++) H[j+nv*nx]=j<3?-e[j]:(j==3?1.0:0.0);
        }
        /* time system and receiver bias offset correction */
        if      (sys==SYS_GLO) {v[nv]-=x[igl]; H[igl+nv*nx]=1.0; mask[1]=1;}
        else if (sys==SYS_GAL) {v[nv]-=x[iga]; H[iga+nv*nx]=1.0; mask[2]=1;}
        else if (sys==SYS_CMP) {v[nv]-=x[icp]; H[icp+nv*nx]=1.0; mask[3]=1;}
        else mask[0]=1;
...
static int valsol(const double *azel, const int *vsat, int n,const prcopt_t *opt,
                  const double *v, int nv, int nx,char *msg)
{
                  ...
                  double *resp, char *msg)
{
    ...
    x=zeros(nx,1); dx=zeros(nx,1); Q=zeros(nx,nx);
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,nx)<1E-4) {
            ...
            for (j=0;j<3;j++) var[j]=SQRT(Q[j+j*nx]);
...
extern int pntpos(const obsd_t *obs, int n, const nav_t *nav,const prcopt_t *opt,
                  sol_t *sol,insstate_t *ins,double *azel,ssat_t *ssat,char *msg)
{
    prcopt_t opt_=*opt;
