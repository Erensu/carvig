...
static void writesol(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<2;i++) {
        if (svr->solopt[i].posf==SOLF_STAT) { /* solution status */
            ...
            /* output solution */
            n=outsols(buff,&svr->rtk.sol,svr->rtk.rb,svr->solopt+i,opt->mode>=PMODE_INS_UPDATE?&svr->rtk.ins:NULL,
...
static int inputpose(rtksvr_t *svr,gtime_t time,pose_meas_t *pose)
{
    ...
    for (i=0;i<np+1;i++) {
        ...
        if (dt&&fabs(dt)<fabs(timediff(time,svr->pose[j].time))) break;

        if (fabs((dt=timediff(time,svr->pose[j].time)))<DTTOL
            &&svr->pose[j].time.time&&svr->pose[j].stat&&norm(svr->pose[j].rpy,3)>0.0) {
...
/* update input stream suspend flag-------------------------------------------*/
...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        for (i=0;i<7;i++) {
            ...
            if ((n=strread(svr->stream+i,p,q-p))<=0) continue;
...
*-----------------------------------------------------------------------------*/
extern int carvigsvrinit(rtksvr_t *svr)
{
    gtime_t time0={0};
    sol_t  sol0 ={{0}};
    eph_t  eph0 ={0,-1,-1};
...
* return : status (1:ok 0:error)
*-----------------------------------------------------------------------------*/
extern int carvigsvrstart(rtksvr_t *svr, int cycle, int buffsize, int *strs,
...
* return : none
*-----------------------------------------------------------------------------*/
extern void carvigsvrstop(rtksvr_t *svr, char **cmds)
{
    int i;
    
    tracet(3,"carvigsvrstop:\n");
...
* return : status (1:ok 0:error)
*-----------------------------------------------------------------------------*/
extern int carvigsvropenstr(rtksvr_t *svr, int index, int str, const char *path,
                            const solopt_t *solopt)
{
    tracet(3,"carvigsvropenstr: index=%d str=%d path=%s\n",index,str,path);
...
* return : none
*-----------------------------------------------------------------------------*/
extern void carvigsvrclosestr(rtksvr_t *svr, int index)
{
    tracet(3,"carvigsvrclosestr: index=%d\n",index);
...
*-----------------------------------------------------------------------------*/
extern int carvigsvrostat(rtksvr_t *svr, int rcv, gtime_t *time, int *sat,
                          double *az, double *el, int **snr, int *vsat)
{
    int i,j,ns;
    
    tracet(4,"carvigsvrostat: rcv=%d\n",rcv);
...
* return : status (1:ok 0:error)
*-----------------------------------------------------------------------------*/
extern int carvigsvrmark(rtksvr_t *svr, const char *name, const char *comment)
{
    char buff[MAXSOLMSG+1],tstr[32],*p,*q;
    double tow,pos[3];
    int i,sum,week;
    
    tracet(4,"carvigsvrmark:name=%s comment=%s\n",name,comment);
