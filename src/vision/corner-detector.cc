/*------------------------------------------------------------------------------
* corner-detector.h : image corner detector functions
*
* version : $Revision:$ $Date:$
* history : 2018/11/09 1.0  new (first version)
*-----------------------------------------------------------------------------*/
#include <corner-detector.h>
#include <math-unit.h>
#include <fast/fast.h>
...
namespace vig {
    ...
    /**
     * @brief construct for CornerDetector class
     * @param n_rows
     * @param n_cols
     * @param detection_threshold
     */
    CornerDetector::CornerDetector(int n_rows,int n_cols,
        ...
        occupancy_grid.resize(grid_n_rows*grid_n_cols,false);
    }
    /**
     * @brief set grid size for corner detector
     * @param n_rows rows of grid size for corner detector
     * @param n_cols clos of grid size for corner detector
     */
    ...
    int CornerDetector::sub2ind(const cv::Point2f& sub) {
        return static_cast<int>(sub.y/grid_height)*grid_n_cols+
               static_cast<int>(sub.x/grid_width);
    }
    void CornerDetector::zero_occupancy_grid()
    {
        std::fill(occupancy_grid.begin(),occupancy_grid.end()-1,false);
    }
    void CornerDetector::set_grid_position(const cv::Point2f& pos) {
        occupancy_grid[sub2ind(pos)]=true;
    }
    float
    ...
    /**
     * @brief deetct feature from image
     * @param image input image buffer
     * @param features get features of detecting
     */
    void CornerDetector::detect_features(const cv::Mat& image, std::vector<cv::Point2f>& features)
    {
        grid_height=(image.rows/grid_n_rows)+1;
        grid_width =(image.cols/grid_n_cols)+1;

        features.clear();
        std::vector<double> score_table(grid_n_rows*grid_n_cols);
        std::vector<cv::Point2f> feature_table(grid_n_rows*grid_n_cols);
        std::vector<fast::fast_xy> fast_corners;

#ifdef __SSE2__
        fast::fast_corner_detect_10_sse2((fast::fast_byte*)image.data,image.cols,image.rows,image.cols,20,fast_corners);
#else
        fast::fast_corner_detect_10(
          (fast::fast_byte*) image.data, image.cols, image.rows, image.cols, 20, fast_corners);
#endif
        ...
        for(auto it:nm_corners) {
            ...
            if (xy.x>=grid_n_cols*grid_width||xy.y>=grid_n_rows*grid_height)
                continue;
        ...
        for (int i=0;i<score_table.size();i++) {
            if (score_table[i] > detection_threshold) {
    ...
    CornerTracker::CornerTracker(int window_size,double min_eigen_threshold,
                                 int max_level,
            ...
            : window_size(window_size, window_size),
              ...
              max_level(max_level),
              termination_criteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS,
                                   termcrit_max_iters,
                                   termcirt_epsilon)
    {
    }
    void CornerTracker::configure(double window_size_,
                                  double min_eigen_threshold_,
                                  int max_level_,
                                  int termcrit_max_iters,
                                  double termcirt_epsilon)
    {
        window_size= cv::Size(window_size_, window_size_);
        min_eigen_threshold = min_eigen_threshold_;
        max_level = max_level_;
        termination_criteria = cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS,
                                                termcrit_max_iters,
                                                termcirt_epsilon);
    }
    void CornerTracker::track_features(cv::Mat img_1, cv::Mat img_2,
    ...
    void TrackVisualizer::add_predicted(Point2fVector& features, IdVector& feature_ids)
    {
        ...
        for (;fit!=features.end() && idit!=feature_ids.end();++fit, ++idit){
            predicted_pts.emplace(*idit, *fit);
        }
    }
    void TrackVisualizer::add_current_features(Point2fVector& features, IdVector& feature_ids)
    {
        ...
        for(;fit!=features.end() && idit!=feature_ids.end();++fit, ++idit){
            ...
            if (feature_tracks.find(id)==feature_tracks.end()){
                feature_tracks.emplace(*idit, Point2fVector());
            }
            feature_tracks[id].push_back(*fit);
        }
        std::vector<int> to_remove;
        for (auto& track : feature_tracks) {
            if (current_ids.count(track.first)==0) to_remove.push_back(track.first);
        }
    ...
    void TrackVisualizer::add_new_features(Point2fVector& features, IdVector& feature_ids)
    {
        ...
        for(;fit!=features.end();++fit, ++idit) {
            ...
            if (feature_tracks.find(id)==feature_tracks.end()){
                feature_tracks.insert(std::make_pair(id, Point2fVector()));
            }
            feature_tracks[id].push_back(*fit);
        }
    }
    cv::Mat TrackVisualizer::draw_tracks(cv::Mat image)
    {
        ...
        for(auto& track : feature_tracks) {
            ...
            auto search = predicted_pts.find(id);
            if (search!=predicted_pts.end()){
              ...
              use_gyro_(true)
    {
        ...
        clear_tracks();
        tracker_.configure(51,0.00001,4,30,1.0);
    }
    ...
    void TrackHandler::tracked_features(OutFeatureVector& features, IdVector& feature_ids)
    {
        ...
        if (cur_features_.size()>0) {
            ...
            std::transform(undistorted_prev_pts.begin(), undistorted_prev_pts.end(), std::back_inserter(prev_features),
                           [](const cv::Point2f& pt){return vig::Vector2<float>{ pt.x, pt.y };});
            ...
            vig::Matrix3<float> dR;
            for (int i=0;i<3;i++) {
                for (int j=0;j<3;j++) dR(i,j)=dR_.at<float>(i,j);
            }
    ...
    void TrackHandler::new_features(OutFeatureVector& features, IdVector& feature_ids) {
        ...
        if (new_features_.size()) {
            ...
            std::transform(undistorted_pts.begin(),
                           undistorted_pts.end(),
                           std::back_inserter(features),
                           [](const cv::Point2f& pt){return vig::Vector2<float>{ pt.x,pt.y};});
    ...
    TrackHandler::twoPointRansac(const vig::Matrix3<float>& dR,
                                 ...
                                 const OutFeatureVector& new_points_in)
    {
        ...
        for (int i=0;i<num_iters;i++) {
            ...
            const vig::Vector3<float> c1=new_points.col(ind1);
            const vig::Vector3<float> c2=new_points.col(ind2);

            M << (dR*old_points.col(ind1)).transpose()*vig::vectorToSkewSymmetric(c1),
                 (dR*old_points.col(ind2)).transpose()*vig::vectorToSkewSymmetric(c2);
            ...
            vig::Matrix3<float> E=vig::vectorToSkewSymmetric(t)*dR;
